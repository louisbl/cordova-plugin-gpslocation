...
exports.defineAutoTests = function () {
	...
	describe('Geolocation (GPSLocation)', function () {

		it("geolocation.spec.1 should exist", function () {
			expect(GPSLocation).toBeDefined();
		});

		it("geolocation.spec.2 should contain a getCurrentPosition function", function () {
			expect(typeof GPSLocation.getCurrentPosition).toBeDefined();
			expect(typeof GPSLocation.getCurrentPosition == 'function').toBe(true);
		});

		it("geolocation.spec.3 should contain a watchPosition function", function () {
			expect(typeof GPSLocation.watchPosition).toBeDefined();
			expect(typeof GPSLocation.watchPosition == 'function').toBe(true);
		});

		it("geolocation.spec.4 should contain a clearWatch function", function () {
			expect(typeof GPSLocation.clearWatch).toBeDefined();
			expect(typeof GPSLocation.clearWatch == 'function').toBe(true);
		});
	...
	describe('getCurrentPosition method', function () {
		...
		describe('success callback', function () {
			...
			it("geolocation.spec.6 should be called with a Position object", function (done) {
				GPSLocation.getCurrentPosition(function (p) {
...
exports.defineManualTests = function (contentEl, createActionButton) {
	var newGeolocation = GPSLocation;
	...
	var location_div = '<div id="info">' +
		...
		'</div>',
		actions =
		'<h2>Use Built-in WebView GPSLocation</h2>' +
	...
	contentEl.innerHTML = values_info + location_div + latitude + longitude + altitude + accuracy + heading + speed + altitude_accuracy + time + note + actions;
